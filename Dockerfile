...
RUN apt-get update && \
apt-get upgrade -y && \
apt-get install -y --no-install-recommends git ca-certificates build-essential cmake gnutls-bin libgnutls28-dev uuid-dev wget && \
...
ADD bin/add-user.sh /usr/local/bin/addUser.sh
RUN chmod +x /usr/local/bin/addUser.sh
ADD bin/server.sh /usr/local/bin/server.sh
