...
class TestHelpers
  include TaskwarriorNagger::App::Helpers
end

describe TaskwarriorNagger::App::Helpers do
  ...
  describe '#format_date' do
    context 'with no format specified' do
      it 'should format various dates and times to the default format' do
        allow(TaskwarriorNagger::Config).to receive(:dateformat).and_return(nil)
    ...
    context 'with a specified date format' do
      it 'should format dates using the specified format' do
        allow(TaskwarriorNagger::Config).to receive(:dateformat).and_return('%d/%-m/%Y')
      ...
      it 'should convert Taskwarrior formats to Ruby formats correctly' do
        allow(TaskwarriorNagger::Config).to receive(:store).and_return({ 'dateformat' => 'd/m/Y' })
  ...
  describe '#colorize_date' do
    context 'when no timestamp is given' do
      it 'should simply return' do
        TaskwarriorNagger::Config.should_not_receive(:due)
    ...
    context 'with a due setting specified' do
      before do
        allow(TaskwarriorNagger::Config).to receive(:due).and_return(3)
      end
      ...
      it 'should return "success" when a date is within the specified range' do
        allow(TaskwarriorNagger::Config).to receive(:due).and_return(5)
    ...
    context 'with no due setting specified' do
      before do
        allow(TaskwarriorNagger::Config).to receive(:due).and_return(nil)
      end
